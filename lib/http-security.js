...
/** HTML, JS e CSS da SPA não devem ficar em cache de CDN após o deploy (arquivos sem hash). */
export function shouldNoStoreAppShell(urlPath) {
  const p = String(urlPath || "").split("?")[0] || "/";
  if (/\.html$/i.test(p)) return true;
  if (HTML_NO_STORE.has(p)) return true;
  if (p === "/sw.js" || p === "/manifest.json") return true;
  if (p.startsWith("/js/") && /\.(js|css)$/i.test(p)) return true;
  return false;
}
...
export function applyBrowserSecurityHeaders(res, req) {
  ...
  if (req && shouldNoStoreAppShell(req.path || req.url)) {
